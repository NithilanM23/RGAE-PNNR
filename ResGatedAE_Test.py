...
def load_dinov2_model():
    ...
    if _feature_extractor is not None: return _feature_extractor
...
def run_evaluation(category):
    ...
    model_path = os.path.join(model_save_dir, "VisBank3Mac1GatedInp_train.pth")
    bank_path = os.path.join(model_save_dir, "VisBank3Mac1Gatedcpr_bank.npy")
    ...
    if not all(os.path.exists(p) for p in [model_path, bank_path]):
        print(f"ERROR: Model files not found in '{model_save_dir}'.")
        return
    ...
    model = INP_Former(feature_dim, grid_h, grid_w).to(DEVICE)
    ...
    # NEW: Aggregator
    # =========================
    aggregator = AnomalyAggregator().to(DEVICE)
    aggregator.eval()

    bank_raw = np.load(bank_path)
    bank_normed = bank_raw / (np.linalg.norm(bank_raw, axis=1, keepdims=True) + 1e-8)
    ...
    for img_path in tqdm(good_images + bad_images, desc=f"Testing {category}", leave=False):
        ...
        with torch.no_grad():
            ...
            _, recon_inp_t = model(t)
            recon_inp_np = recon_inp_t.squeeze(0).permute(1, 2, 0).cpu().numpy()

        recon_cpr_np = cpr_reconstruct_from_bank(feat_np, bank_normed, bank_raw)
        anomaly_map = np.mean((recon_inp_np - recon_cpr_np) ** 2, axis=2)

        # =========================
        # CHANGED: Image-level score
        # =========================
        with torch.no_grad():
            amap_t = torch.from_numpy(anomaly_map).to(DEVICE)
            image_score = aggregator(amap_t).item()

        image_scores.append(image_score)
        image_labels.append(1 if is_bad else 0)

        # =========================
        # Pixel-level (UNCHANGED)
        # =========================
        ...
        if is_bad:
            mask_path = os.path.join(
                mask_dir, os.path.basename(img_path).replace(".JPG", ".png")
            )
    ...
    print("Concatenating pixel data...")
    all_pixel_preds_np = np.concatenate(all_pixel_preds)
    all_pixel_labels_np = np.concatenate(all_pixel_labels)
    
    print(f"Total pixels: {len(all_pixel_labels_np)}. Subsampling to {PIXEL_SUBSAMPLE_PERCENT*100}%...")
    num_pixels = len(all_pixel_labels_np)
    subsample_size = int(num_pixels * PIXEL_SUBSAMPLE_PERCENT)
    rng = np.random.RandomState(42)
    subsample_indices = rng.choice(num_pixels, subsample_size, replace=False)
    
    pixel_preds_sub = all_pixel_preds_np[subsample_indices]
    pixel_labels_sub = all_pixel_labels_np[subsample_indices]
    
    print("Calculating pixel-level AUC on subsampled data...")
    pixel_auc = roc_auc_score(pixel_labels_sub, pixel_preds_sub)
    ...
    print(f"{'Pixel-level AUC':<20} | {pixel_auc:.4f} (estimated from {PIXEL_SUBSAMPLE_PERCENT*100}% of pixels)")
    print("="*50)
